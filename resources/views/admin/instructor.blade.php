...
<main class="instructors-main">
  <div class="container-fluid px-5 py-4">
    ...
    <div class="d-flex justify-content-between align-items-start mb-2">
      ...
      <div>
        <button class="btn px-3 py-2 fw-semibold" style="background-color: #FDAF22; border: none; color: white;" onclick="openCreateInstructorModal()">
          <i class="fa-solid fa-plus me-2"></i> Create New Instructor
        </button>
      </div>
    </div>
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4" style="background: #f9f9f9; border: 1px solid #e8e8e8;">
          ...
          <tbody id="instructorsTableBody">
            <tr style="border-bottom: 1px solid #e8e8e8;">
              <td colspan="8" class="text-center text-muted py-4">
                <i class="fa-solid fa-spinner fa-spin me-2"></i>Loading instructors...
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div class="d-flex justify-content-between align-items-center mt-5 pt-4" style="border-top: 1px solid #e8e8e8;">
        <!-- Previous Button -->
        <button class="btn px-4 py-2" id="prevBtn" onclick="loadInstructors(currentPage - 1)" style="border: 1px solid #004A53; color: #004A53; font-weight: 500; border-radius: 0.5rem;" disabled>
        ...
        <!-- Next Button -->
        <button class="btn px-4 py-2" id="nextBtn" onclick="loadInstructors(currentPage + 1)" style="border: 1px solid #004A53; color: #004A53; font-weight: 500; border-radius: 0.5rem;">
...
<script>
  ...
  async function loadInstructors(page = 1) {
    try {
      let url = `/api/admin/users?page=${page}&per_page=20&role=instructor`;
      ...
      if (data.success && data.data) {
        ...
        if (instructors.length === 0) {
          tbody.innerHTML = '<tr><td colspan="8" class="text-center text-muted py-4">No instructors found</td></tr>';
        } else {
              ...
              <tr style="border-bottom: 1px solid #e8e8e8;">
                ...
                <td style="padding: 1rem;">
                  <div class="d-flex align-items-center">
                    ...
                    <span style="color: #333; font-weight: 500;">${instructor.first_name} ${instructor.last_name}</span>
                  </div>
                </td>
                <td style="padding: 1rem; color: #666;">KOKOKAH-${String(instructor.id).padStart(4, '0')}</td>
                <td style="padding: 1rem; color: #666;">${instructor.email}</td>
                <td style="padding: 1rem;"><span style="color: #666;">${instructor.gender || 'N/A'}</span></td>
                <td style="padding: 1rem; color: #666;">${instructor.contact || 'N/A'}</td>
                <td style="padding: 1rem; color: #666;">${instructor.address || 'N/A'}</td>
                <td style="padding: 1rem;">
                  <div class="d-flex gap-2">
                    <a href="/edituser?id=${instructor.id}" class="btn btn-sm btn-light" style="border: 1px solid #ddd; padding: 0.5rem 0.75rem;" title="Edit">
                      <i class="fa fa-edit" style="color: #004A53;"></i>
                    </a>
                    <button class="btn btn-sm btn-light delete-btn" data-user-id="${instructor.id}" style="border: 1px solid #ddd; padding: 0.5rem 0.75rem;" title="Delete">
  ...
  function generatePageNumbers(current, total) {
    ...
    if (endPage < total) {
      ...
      btn.onclick = () => loadInstructors(total);
      pageNumbersDiv.appendChild(btn);
    }
  }
  ...
  async function handleDelete(e) {
    ...
    if (confirm('Are you sure you want to delete this instructor? This action cannot be undone.')) {
      try {
        ...
        showAlert('An error occurred while deleting the instructor', 'error');
        btn.disabled = false;
        btn.innerHTML = '<i class="fa fa-trash" style="color: #dc3545;"></i>';
      }
    }
  ...
  function openCreateInstructorModal() {
    alert('Create New Instructor feature coming soon!');
  }
</script>
